...
- `App/Views/Overview/MealOverlayLogic.swift` — extracted `mealImpactDeltaColor(delta:)` free function (thresholds: <30 → cgaGreen, 30–59 → amber, ≥60 → cgaRed) as the single tier source; FoodImpactView is its first caller (the overlay can adopt it later)
...
**Delta tier color extracted** — the 30/60 mg/dL thresholds were only inline assertions in `MealImpactTests.swift`. Extracted to `mealImpactDeltaColor(delta:)` in `MealOverlayLogic.swift` as the single source of truth for the 30/60 bands. FoodImpactView is currently its only caller — the existing overlay renders deltas untinted today and can adopt the helper in a future pass.
