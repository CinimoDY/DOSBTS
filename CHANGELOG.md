...
### Fixed
- Sensor status line: the trailing chip (DISCONNECT/CONNECT/SET UP) no longer overlaps long sensor labels — the label's reserved width is now measured from the chip's actual rendered size instead of a hard-coded constant that didn't match any chip — DMNC-1481, PR #112
- Daily Digest timeline event colours now match the Overview event marker lane (meal green, insulin amber/light/dark by sub-type, exercise cyan) instead of a mismatched, cyclically-permuted set — DMNC-1481, PR #112
...
### Changed
- Insulin entry sheet matches the standard entry chrome (system nav bar with Cancel / title / Add) — DMNC-1312, PR #95
- Decorative "> " prefixes removed from meal section labels (QUICK, RECENT, OTHER) and recent meal rows — DMNC-1312, PR #95
...
### Changed
- Event marker lane now distinguishes correction boluses from meal/snack boluses: correction shows a bright-amber "c" suffix (e.g. `3.0Uc`) so you can tell at a glance whether you injected for a meal or to correct a high — DMNC-715
- Scored meals in the event marker lane show a `★` prefix (e.g. `★45g`) making it immediately obvious which entries have a glycemic impact score vs those still pending — DMNC-715
- Event marker chips keep all insulin types (meal/snack bolus, correction, basal) on a single insulin row so a busy 15-minute window stays within the marker lane instead of overflowing — DMNC-715
...
### Added
- Persistent log actions on every tab: the INSULIN/MEAL buttons now live in a bar above the tab bar (same DOS ghost styling, icon beside label) and work from Overview, Log, Settings, and Daily digest. During a hypo treatment cycle, MEAL opens the hypo-filtered entry directly (R7, R8, AE2) — PR #52.
- Slim glucose strip at the top of the Log, Settings, and Daily digest tabs: current value with trend in the hero's colors, a timer indicator during treatment cycles, and the same 5–14 min amber / 15+ min red stale warnings as the hero — one shared staleness source, no separate refresh path (R7b) — PR #52.

### Changed
- Overview's own INSULIN/MEAL button row is gone — the persistent bar replaces it (no double buttons), and the chart expands into the reclaimed space (R9 Phase 2, taken early by user decision) — PR #52.
- All sheets (entry, treatment, chart-marker) now present from a single app-level root, so treatment prompts can never collide with or hide behind a user-opened sheet, and they now land you on the Overview tab where the treatment banner lives — even if they fire while you're in Settings or Digest (R8a, AE2a) — PR #52.
- Small informational text (settings descriptions, empty states, timestamps, field labels, digest statuses) brightened from dim amber to full amber across the app and widgets — dim amber measured 3.7:1 on black, below the WCAG AA floor. Dimming for disabled controls, unselected tabs, and decorative strokes is unchanged (R5/AE3) — PR #52.
- Meal rows in the food-entry recents and the Lists tab now share one component. The Lists tab gains an explicit Delete swipe action and Delete in the long-press menu (deleting previously required edit mode) — R3, R4, AE4 — PR #52.
- True CGA monitor feel: no real white anywhere the app controls. All unstyled text falls back to phosphor amber + monospace, navigation titles render amber-light monospace, toolbar buttons tint amber, and the manual Meal, Blood glucose, and Calibration forms plus the AI analysis screen drop their system-gray surfaces for black/amber (R1, R1a, AE1). CGA accents (green/cyan/red) and system chrome (keyboard, picker internals, back buttons) are unchanged — PR #52.

### Fixed
- Overview no longer overflows the screen when the hypo-treatment countdown banner is active — the chart compresses to absorb the banner, so the INSULIN/MEAL buttons stay fully above the tab bar instead of being pushed behind it — PR #52.
- ASK AI in food search now navigates and starts the analysis on the first tap (it previously took several taps and often did nothing): the row's navigation cancelled itself because starting the analysis removed the link mid-push. The keyboard now drops on tap and the search text is kept for when you navigate back — DMNC-1023 wave (R6) — PR #52.
- "Log again" on a recent meal navigates again — its navigation destination was silently ignored inside the legacy navigation container — PR #52.
...
### Added
- "> QUICK" header is now tappable with a chevron: expands the favourites row into a wrapped grid showing all favourites (the row previously capped at 8 with no way to see the rest) — DMNC-796.
